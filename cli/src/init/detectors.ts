...
import type { StackInfo, DetectedTool } from '../types.js';

/**
 * Detect the project stack (language, runtime, tools)
 */
export async function detectStack(cwd: string = process.cwd()): Promise<StackInfo> {
 ...
 */
function detectJsRuntime(cwd: string): 'bun' | 'deno' | 'node' {
  if (
 ...
 */
async function detectJsTools(cwd: string, stack: StackInfo): Promise<void> {
  const runtime: 'bun' | 'deno' | 'node' = stack.runtime ?? 'node';
 ...
 * Detect Python tools
 */
async function detectPythonTools(cwd: string, stack: StackInfo): Promise<void> {
 ...
 * Detect Rust tools
 */
async function detectRustTools(cwd: string, stack: StackInfo): Promise<void> {
 ...
 * Detect Go tools
 */
async function detectGoTools(cwd: string, stack: StackInfo): Promise<void> {
 ...
 * Detect Java tools
 */
async function detectJavaTools(cwd: string, stack: StackInfo): Promise<void> {
 ...
 */
export function getStackSummary(stack: StackInfo): string {
  const parts: string[] = [];
